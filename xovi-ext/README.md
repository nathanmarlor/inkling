# scribefb — page-clear xovi extension
...
```
[scribefb] loaded (page-clear via SceneController)
[scribefb] cleared page (1 controller(s), 3 view(s))
```

## What it does

On `touch /tmp/scribefb_clear`, a worker thread finds the active page's scene controller
...
```sh
touch /tmp/scribefb_clear      # scribefb picks it up within ~120ms and deletes it
```
...
`scribefb.xovi` declares no imports/overrides, so xovi auto-loads it with plain
`LD_PRELOAD`.
...
scp scribefb.so root@<tablet-ip>:/home/root/xovi/extensions.d/
...
systemctl restart xochitl        # xovi auto-loads scribefb
touch /tmp/scribefb_clear        # clears the current page
```
